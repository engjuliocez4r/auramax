...
Cada cenário não é um único duelo contra um adversário — é UMA sequência contínua de 10 rounds dentro do mesmo cenário, com o mesmo duelo a decorrer sem interrupção de jogabilidade.
...
Razão de produção: com muitos cenários pela frente, dar um cosmético por adversário individual não escala. Com chefe de cenário, o custo de arte fica fixo — um cosmético por cenário, não um por round — mesmo com centenas de rounds no total do jogo.
O tempo REINICIA a cada round (ponto 17, aplicado por round e não ao cenário inteiro).
A dificuldade continua a viver nos INTERVALOS entre limiares consecutivos, agora dentro do mesmo cenário em vez de entre adversários separados.
...
- **Tap/streak/aura/intensity/burst não foram tocados** nesta reestruturação — só a bookkeeping de progressão da história por cima (CLAUDE.md Regra 3).
- **CLAUDE.md Regra 1 aplicada a sério desta vez**: `Scenario` é um script `class_name` escrito fora do editor. TODAS as referências a instâncias de `Scenario` — em `duel.gd`, `scripts/result_screen.gd`, `scripts/boss_card.gd` e `scripts/boss_defeat_screen.gd` — são variáveis/parâmetros sem tipo (`var scenario = ...`), nunca `: Scenario`, com comentário a apontar para a regra. Verificado com `grep` a toda a `scripts/` por `: Scenario`, `-> Scenario`, `Array[Scenario]` antes de terminar: zero ocorrências fora dos comentários explicativos em `scenario.gd` e nos três scripts acima.
...
- `scenes/home.tscn` / `scripts/home.gd` — agora a **cena principal do projeto** (substituiu `duel.tscn` em `project.godot`). Nome do jogador (ou título do jogo, se vazio) sobre uma barra de ego fina, avatar placeholder (`ColorRect`), botões História/Arcade lado a lado e O Quarto por baixo, ícone de definições no canto. `home.gd` não tem `class_name`, pela mesma razão documentada em `duel.gd`: é o script raiz da cena principal, analisado antes de o cache global de classes estar garantidamente completo (CLAUDE.md Regra 1).
...
Fundação (tap/streak/aura/burst) confirmada em headless na sessão anterior — ver git log. **Nem o bloco adversário/resultado original nem a reestruturação em cenários/rounds/chefe foram executados em headless**, por pedido explícito do autor; fica para teste manual no editor. A Regra 1 do `CLAUDE.md` foi verificada por `grep` (ver bloco da reestruturação acima), não por correr o projeto.
...
- **`.godot/global_script_class_cache.cfg` ainda não inclui `Scenario`, `BossCard`, `BossDefeatScreen`** (nem `BackHomeButton`/`TheRoomScreen`/`SettingsScreen`, do trabalho anterior) — só passa a incluir depois de o projeto ser aberto a sério no editor. Até lá, nenhum script pode tipar contra eles (CLAUDE.md Regra 1); todos os pontos de acesso atuais já respeitam isto.
...
7. **Abrir o projeto no editor real** para confirmar que `Scenario`/`BossCard`/`BossDefeatScreen` entram no cache de classes sem erro, e só depois considerar tipar contra eles se algum dia isso trouxer benefício real (CLAUDE.md Regra 1).
