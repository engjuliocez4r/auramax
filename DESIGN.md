...
- O primeiro toque depois de qualquer reset de streak conta sempre, seja qual for o lado — MAS só se o reset foi causado pelo TIMEOUT de inatividade. Um reset causado por REPETIR O MESMO LADO nunca limpa o último lado registado: se limpasse, martelar sempre o mesmo lado passaria a conceder aura em toques alternados (o segundo toque "pareceria" um recomeço válido). São duas regras que passam pela mesma função (`_reset_streak`) mas com efeitos diferentes sobre `_last_side` — distinguidas por um parâmetro, não por duplicar a função.
