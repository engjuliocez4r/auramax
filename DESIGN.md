...
Os anéis do PowerRing ficaram ainda mais discretos: ligeiramente ovais (mais largos que altos, um esticão subtil), nascem completamente fora do ecrã e primeiro deslizam para dentro antes de começarem a contrair — nunca as duas coisas ao mesmo tempo — e fazem-no visivelmente mais depressa do que as partículas convergem, criando uma sensação de profundidade (paralaxe) entre as duas camadas. A opacidade máxima foi reduzida substancialmente: os anéis são agora uma sugestão discreta de energia que o jogador quase repara, nunca um elemento gráfico a competir com as partículas.
